...
router.post("/", adminAuth, async (req, res) => {
    ...
    if (!version) {
        version = new Version(_.pick(req.body, ["major", "minor", "patch", "appType", "forceUpdateTitle", "forceUpdateMessage",
            "optionalUpdateTitle", "optionalUpdateMessage", "updateType", "showAppMaintainace", "isStickOnScreen",
            "appMaintainanceMesageTitle", "appMaintainanceMesageBody", "isShowImage", "imageUrl"]));
    } else {
    ...
    await version.save();
    return res.send({ statusCode: 200, message: "Success", data: VERSION_CONSTANT.SUBMIT_SUCCESS });
});
...
router.get("/check", async (req, res) => {
    ...
    if (!req.query.v) {
        ...
        return res.send({ statusCode: 200, message: "Success", data: { optionalUpdate: false, forceUpdate: false, message: VERSION_CONSTANT.APPTYPE_MANDATORY } });
    }
